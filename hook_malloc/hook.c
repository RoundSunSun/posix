...
// gcc -o hook.so -g -fPIC -shared hook.c -ldl -lpthread
...
void *malloc(size_t size)
{
    ...
    if (policy == SCHED_FIFO && guard == 0){
        ...
        // backtrace
        void *callstack[128];
        int frames = my_backtrace(callstack, 50);
        ...
        for (int i = 1; i < frames; i++) {
            log_buffer_offset += snprintf(log_buffer + log_buffer_offset, sizeof(log_buffer) - log_buffer_offset, "CALL STACK:  %s\n", symbols[i]);
        }
        free(symbols);
